...
'''
def list2txt(save_path, save_list):
    save_list = list(map(lambda x: x[:-4], save_list))    
    with open(save_path, 'w+') as file:
        file.write('\n'.join(save_list))

list2txt(os.path.join(SAVE_GALLERY, 'val_id.txt'), os.listdir(GALLERY))
list2txt(os.path.join(SAVE_TRAIN, 'val_id.txt'), os.listdir(TRAIN))
list2txt(os.path.join(SAVE_QUERY, 'val_id.txt'), os.listdir(QUERY))
